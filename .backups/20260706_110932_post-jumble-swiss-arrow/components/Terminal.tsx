...
function useScrambleText(text: string, delay: number = 0, triggerKey: number = 0): string {
  const [display, setDisplay] = useState('');

  useEffect(() => {
    // Start with randomized characters (spaces stay as spaces)
    setDisplay(
      text
        .split('')
        .map((ch) =>
    ...
    const startTimeout = setTimeout(() => {
      // Scramble: randomize unsettled characters every 30ms
      ...
      // Lock in characters left-to-right with 40-80ms stagger
      const lockOne = () => {
      ...
      // Use variable timing: 40-80ms per character lock
      const scheduleNext = () => {
    ...
    // Re-run when triggerKey changes (expand from collapsed replays the animation)
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = '#9B9AA0'; }}
      >
