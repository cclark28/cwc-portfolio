...
function scatterItems(
  ...
  sorted.forEach((item, i) => {
    ...
    placed.push({ x, y, w: item.w, h: item.h + 96, id: item.id });
  });

  // Close any large horizontal or vertical gaps so removed cards don't leave holes.
  // Scan columns: group cards into vertical strips and shift strips left if there's a big gap.
  const ids = Object.keys(positions);
  if (ids.length > 2) {
    // Sort all placed cards by x position
    const byX = ids.slice().sort((a, b) => positions[a].x - positions[b].x);
    const avgW = sorted.reduce((s, i) => s + i.w, 0) / count;
    const maxGap = cellW * 1.3; // anything wider than ~1.3 cells is a hole

    for (let i = 1; i < byX.length; i++) {
      const prev = byX[i - 1];
      const curr = byX[i];
      const prevRight = positions[prev].x + (sorted.find(s => s.id === prev)?.w ?? avgW);
      const gap = positions[curr].x - prevRight;

      if (gap > maxGap) {
        // Shift this card and all cards to its right, closing the gap
        const shift = gap - cellW * 0.5;
        for (let j = i; j < byX.length; j++) {
          positions[byX[j]].x -= shift;
        }
      }
    }

    // Same for vertical gaps
    const byY = ids.slice().sort((a, b) => positions[a].y - positions[b].y);
    const avgH = sorted.reduce((s, i) => s + i.h + 96, 0) / count;
    const maxGapY = cellH * 1.3;

    for (let i = 1; i < byY.length; i++) {
      const prev = byY[i - 1];
      const curr = byY[i];
      const prevBottom = positions[prev].y + (sorted.find(s => s.id === prev)?.h ?? avgH) + 96;
      const gap = positions[curr].y - prevBottom;

      if (gap > maxGapY) {
        const shift = gap - cellH * 0.5;
        for (let j = i; j < byY.length; j++) {
          positions[byY[j]].y -= shift;
        }
      }
    }
  }
