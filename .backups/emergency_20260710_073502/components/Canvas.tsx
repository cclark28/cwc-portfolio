...
const Canvas = forwardRef<CanvasHandle, CanvasProps>(function Canvas(
  ...
  // Items visible on canvas: playground only when playground filter is active
  const visibleItems = useMemo(
    () => filter === 'playground' ? playground : [...work, ...photo],
    [work, photo, playground, filter]
  );
  ...
  const recompute = useCallback(
    (nextScale?: number, nextFilter?: Category | null, goToPile?: boolean) => {
      ...
      // Default layout excludes playground — playground only appears when filtered
      const defaultItems = [...work, ...photo];
      const newFullLayout = computeLayout(work, photo, vw, cardGap);

      setFullLayout(newFullLayout);

      // Compute pile layout centered in the full layout's centroid (no playground)
      const centroid = computeCentroid(newFullLayout);
      const pile = computePileLayout(defaultItems, centroid.x, centroid.y);
      setPileLayout(pile);

      if (effectiveFilter === 'playground') {
        // Show only playground items in a filtered layout
        const filtLayout = computeFilteredLayout(playground);
        setActiveLayout(filtLayout);
        const view = centerOn(playground, filtLayout, nextScale ?? scale, vw, vh);
        setPan(view.pan);
      } else if (goToPile || (!effectiveFilter && !organized)) {
        // Back to pile or initial state
        setActiveLayout(pile);
        const view = centerOn(defaultItems, pile, nextScale ?? scale, vw, vh);
        setPan(view.pan);
      } else if (effectiveFilter) {
        // Compute a filtered layout for matching items (work or photo)
        const matchingItems = defaultItems.filter((p) => p.category === effectiveFilter);
        ...
        const view = centerOn(defaultItems, newFullLayout, nextScale ?? scale, vw, vh);
        setPan(view.pan);
      }

      if (nextScale !== undefined) setScale(nextScale);
    },
    [work, photo, playground, cardGap, filter, scale, organized, computeCentroid] // eslint-disable-line react-hooks/exhaustive-deps
  );
    ...
    >
      ...
      >
        {visibleItems.map((item, i) => {
          ...
          if (!fullPos && !activePos) return null; // skip items without layout data
          ...
          const rot = (fullPos?.rotation || activePos?.rotation || 0);
          ...
          if (!organized && pileLayout && pileLayout[item.id]) {
            ...
            // Year-based z-index: newer projects stack on top
            const seed = layoutSeedFromId(item.id);
            cardZ = (item.year - 2000) * 10 + Math.round(layoutSeededRandom(seed, 20) * 9);
          } else if (dimmed) {
            ...
            // No layout data available — skip rendering
            return null;
          }
