...
import { centerOn, computeLayout, computeFilteredLayout } from '@/lib/canvasLayout';
...
// Seeded random helper (matches canvasLayout.ts logic)
...
const Canvas = forwardRef<CanvasHandle, CanvasProps>(function Canvas(
  ...
  const scatterDoneRef = useRef(false); // true once the initial burst finishes
  ...
  const recompute = useCallback(
    (nextScale?: number, nextFilter?: Category | null) => {
      ...
      if (effectiveFilter) {
        ...
        // Center on the filtered layout positions
    ...
    [work, photo, playground, all, cardGap, filter, scale, computeCentroid] // eslint-disable-line react-hooks/exhaustive-deps
  );
  ...
  useEffect(() => {
    ...
    // Scatter-in: delay before cards burst from pile to final positions
    const scatterTimer = setTimeout(() => setScattered(true), 120);
    // Mark initial scatter done after burst completes (delay + transition)
    const scatterDoneTimer = setTimeout(() => { scatterDoneRef.current = true; }, 120 + 600 + 550);
    const onResize = () => recomputeRef.current();
    window.addEventListener('resize', onResize);
    return () => {
      clearTimeout(scatterTimer);
      clearTimeout(scatterDoneTimer);
    ...
    >
      ...
      >
        {all.map((item, i) => {
          ...
          // Scatter-in: pile position or final position
          const seed = seedFromId(item.id);
          const pileOffsetX = (seededRandom(seed, 10) - 0.5) * 40; // +/-20px
          const pileOffsetY = (seededRandom(seed, 11) - 0.5) * 40;
          const pileRot = rot * 2; // amplified rotation for messy stack
          ...
          if (!scattered) {
            // Phase 1: Pile at centroid
            cardLeft = centroid.x + pileOffsetX;
            cardTop = centroid.y + pileOffsetY;
            cardRot = pileRot;
            cardOpacity = 0.85;
          } else if (dimmed) {
          ...
          // Stagger delay only during initial scatter burst, not filter changes
          const scatterDelayMs = !scatterDoneRef.current ? Math.min(i * 20, 600) : 0;

          // Build transform
          const cardTransform = dimmed && scattered
          ...
          return (
            <div
              ...
              style={{
                ...
                transitionDelay: !scattered ? '0ms' : `${scatterDelayMs}ms`,
            ...
            >
              <div
                style={{
                  ...
                  animation: scattered && !dimmed && !hovered
